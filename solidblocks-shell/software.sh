...
function software_ensure_shellcheck() {
  ...
  download_get_and_verify_checksum "https://github.com/koalaman/shellcheck/releases/download/${version}/shellcheck-${version}.linux.x86_64.tar.xz" "${CACHE_DIR}/shellcheck-${version}.linux.x86_64.tar.xz" ${checksum}
  download_extract_file_to_directory "${DIR}/.cache/shellcheck-${version}.linux.x86_64.tar.xz" "${BIN_DIR}"
...
function software_ensure_hugo() {
  ...
  download_get_and_verify_checksum "https://github.com/gohugoio/hugo/releases/download/v${version}/hugo_${version}_Linux-64bit.tar.gz" "${CACHE_DIR}/hugo_${version}_Linux-64bit.tar.gz" ${checksum}
  download_extract_file_to_directory "${CACHE_DIR}/hugo_${version}_Linux-64bit.tar.gz" "${BIN_DIR}"
  #touch "${BIN_DIR}/shellcheck-${version}/export.path"
}
